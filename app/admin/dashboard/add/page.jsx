...
export default function AddPropertyPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.id) {
      ...
      formData.id = `${formData.type.substring(0, 3)}-${timestamp}`;
    }
    ...
    const { error } = await insforge.database.from('properties').insert([
      {
        ...
        nearby_places: formData.nearby_places // SDK should handle JSONB
      }
    ]);
        ...
        <form onSubmit={handleSubmit} className="space-y-8 bg-[#161616] border border-white/5 rounded-3xl p-10 shadow-2xl">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div className="space-y-6">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-400 mb-2">Property ID (Optional)</label>
                <input
                  type="text"
                  name="id"
                  value={formData.id}
